...
/***
 * 从LBFGS算法的流程来看，其整个的核心的就是如何快速计算一个Hesse的近似：
 * 重点一是近似，所以有了LBFGS算法中使用前m个近似下降方向进行迭代的计算过程；
 * 重点二是快速，这个体现在不用保存Hesse矩阵上，只需要使用一个保存后的一阶导数序列就可以完成，因此不需要大量的存储，从而节省了计算资源；
 * 重点三，是在推导中使用秩二校正构造了一个正定矩阵，即便这个矩阵不是最优的下降方向，但至少可以保证函数下降。
 * @author Ss
 *
 */
public class LBFGS //大规模优化算法 - LBFGS算法 (拟牛顿法 - BFGS)
